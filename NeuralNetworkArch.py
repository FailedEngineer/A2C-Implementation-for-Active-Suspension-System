...
class ActorNetwork(keras.Model):
    ...
    def call(self, state, training=None):
        # Ensure input is correctly shaped for batch or single instance
...
class ActorCriticAgent(keras.Model):
    """
    STABILIZED Agent Class
    - Adds gradient clipping to prevent exploding gradients during training.
    - Ensures tensors are correctly shaped for batch processing.
    """
    def __init__(self, actor_lr=0.001, critic_lr=0.01, gamma=0.99, action_bound=60.0, grad_clip_norm=0.5):
        ...
        print("--- Using Custom Architectures (Stabilized) ---")
        print(f"Actor parameters: {self.actor.count_params()}")
        print(f"Critic parameters: {self.critic.count_params()}")
        print(f"Gradient Clipping Norm: {self.grad_clip_norm}")
        print("---------------------------------------------")
    
    def get_action(self, state):
        # Prepare state for the network (handle scalar or numpy array)
        if isinstance(state, (int, float, np.float32, np.float64)):
             state = tf.constant([[state]], dtype=tf.float32)
    ...
    def train_step(self, state_tensor, action_tensor, reward_tensor, next_state_tensor, done_tensor):
        ...
        # STABILITY FIX: Clip gradients to prevent them from exploding
        actor_gradients, _ = tf.clip_by_global_norm(actor_gradients, self.grad_clip_norm)
        critic_gradients, _ = tf.clip_by_global_norm(critic_gradients, self.grad_clip_norm)
